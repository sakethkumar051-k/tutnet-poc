            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {tutors.map((t) => {
                    ...
                    return (
                        <div
                            key={t._id}
                            className="group border border-gray-100 rounded-xl p-4 hover:border-indigo-200 hover:shadow-md transition-all duration-200 cursor-pointer"
                            onClick={() => navigate(`/tutor/${t.userId?._id}`)}
                        >
                            {/* Avatar + name */}
                            <div className="flex items-center gap-3 mb-3">
                                <div className="w-11 h-11 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-sm flex-shrink-0 group-hover:bg-indigo-200 transition-colors">
                                    {initials}
                                </div>
                                <div className="min-w-0">
                                    <p className="text-sm font-semibold text-gray-900 truncate">{t.userId?.name}</p>
                            ...
                            {subjects.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mb-3">
                                    {subjects.map((s, i) => (
                                        <span key={i} className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-xs font-medium rounded-md border border-indigo-100">
                                            {s}
                                        </span>
                                    ))}
                                    {t.subjects?.length > 3 && (
